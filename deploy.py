...
# os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'cyberkshetra.settings')
# django.setup()

# from django_celery_beat.models import PeriodicTask, IntervalSchedule
# # from fuzzer.models import TestcaseData
# from accounts.models import Organization, User, PermissionRule
# from django.conf import settings
# from base.management.commands import add_default_organization, custom_createsuperuser, load_permission_rules, \
#     setupapidocs, setupdb, setupldap, setuppermissions, setupproject, setupredis
...
#         6. Create Cache Table (if missing)  
#         7. Start Django Server  
#         8. Start Celery Worker  
#         9. Start Celery Beat  
#     """
#     # Set up redis:
#     call_command('setupredis')
...
#     # Set up LDAP if not exists
#     call_command('setupldap')

#     # Create default organization if not exists
#     if Organization.objects.count() == 0:
#         call_command('add_default_organization')


#     # Load permission rules
#     try:
#         call_command('load_permission_rules')
#     except Exception as e:
#         print(f"Error loading permission rules: {e}")

#     # Setup Permissions
#     try:
#         call_command('setuppermissions', ['-a', 'load'])
#     except Exception as e:
#         print(f"Error setting up permissions: {e}")

#     # Setup API docs
#     call_command('setupapidocs', ['-a', 'load'])

#     # Setup DB
#     call_command(('setupdb',['-a','load']))
    
    

#     # Populate DB if no records exist
#     # if not TestcaseData.objects.exists():
...
#     celery_worker = run_command("celery -A rapifuzz worker --loglevel=info")

#     time.sleep(3)  # Ensure worker starts properly

#     print("Starting Celery Beat...")
#     celery_beat = run_command("celery -A rapifuzz beat --loglevel=info")
